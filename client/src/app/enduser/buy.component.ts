...
export class AppBuy implements OnInit {
      ...
      ngOnInit(){
				...
				this.sharedService.getUserProfile(function(user){
					...
					if(that.userDetail.user_id){
						...
						if(id){
							that.commonService.enduserService.getBuy("",id,"")
							  .subscribe( data => {
							...
							that.commonService.enduserService.getBuy(that.userDetail.user_id,"","")
							  .subscribe( data => {
	  ...
	  onDeactivate(evt){
		  ...
		  this.sharedService.openMessageBox("C","Are you sure you want to deactivate it?",function(flag){
				that.commonService.enduserService.getBuy("",that.item.buy_req_id,"")
						.subscribe( res => {
